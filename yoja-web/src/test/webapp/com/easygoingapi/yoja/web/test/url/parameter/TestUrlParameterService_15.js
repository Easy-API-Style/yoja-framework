...
export default function test(args, resolve, rejet) {
    urlParameterService.onChange(h => {
        if ('before-replace' === h.event) {
            ywAssert.assertArrayEquals(['value_11', 'value_12', 'value_13'], urlParameterService.getAll('key_1'))
        }
        if ('after-replace' === h.event) {
            ywAssert.assertArrayEquals(['value_11', 'value_12', 'value_13'], urlParameterService.getAll('key_1'))
        }
        if ('pop' === h.event) {
    ...
    urlParameterService.append('key_1', 'value_13')
